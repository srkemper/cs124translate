...
class HolbrookCorpus:
  ...
  def read_holbrook(self, data):
    ...
    for sent_list in data:
      sent = ' '.join(str(e) for e in sent_list)
      sentence = self.processLine(sent)      
      if sentence:
  ...
  def vocabulary(self):
    ...
    ret = set()
    for sentence in self.corpus:
      ret |= set(d.word for d in sentence.data[1:-1])
    return ret
